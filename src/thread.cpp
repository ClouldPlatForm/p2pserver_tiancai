...
#include "thread.h"      /* here you can also find the interface to each function */

int CThread::thpool_keepalive = 1;

pthread_mutex_t CThread::mutex = PTHREAD_MUTEX_INITIALIZER;
...
thpool_t* CThread::thpool_init(int threadsN){
	thpool_t* tp_p;
...
void* CThread::thpool_thread_do(void* tp){

	thpool_t* tp_p = (thpool_t*)tp;
...
int CThread::thpool_add_work(thpool_t* tp_p, void *(*function_p)(void*), void* arg_p){
	thpool_job_t* newJob;
...
void CThread::thpool_destroy(thpool_t* tp_p){
	int t;
...
/* Initialise queue */
int CThread::thpool_jobqueue_init(thpool_t* tp_p){
...
/* Add job to queue */
void CThread::thpool_jobqueue_add(thpool_t* tp_p, thpool_job_t* newjob_p){ /* remember that job prev and next point to NULL */
...
int CThread::thpool_jobqueue_removelast(thpool_t* tp_p){
...
thpool_job_t* CThread::thpool_jobqueue_peek(thpool_t* tp_p){
	return tp_p->jobqueue->tail;
}

/* Remove and deallocate all jobs in queue */
void CThread::thpool_jobqueue_empty(thpool_t* tp_p){
